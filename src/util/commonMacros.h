...
template <class T>
inline void write_cio_midline(std::ostream &out, const T &dat,
                              bool isBinary = true) {
#if FORCE_CIO_BINARY
...
template <class T>
inline void write_cio_endline(std::ostream &out, const T &dat,
                              bool isBinary = true) {
#if FORCE_CIO_BINARY
  out.write((char *)&dat, sizeof(T));
  return;
...
template <class T>
inline int ReadSpecificType(istream &in, T &val, string &buf,
                            bool readUntilSuccessful = false,
                            char commentChar = '#',
  ...
  // BLOCK READ -- PLC
  if ((sz >= blockLeftBrac.length()) &&
      buf.compare(0, blockLeftBrac.length(), blockLeftBrac) == 0) {
    char c;
    in.get(c);
    bool brk = false;
    while (!brk) {
