...
template <class T> int vectorOut(ostream &out, const vector<T> &dat, string delim = "{}")
{
    out << delim.at(0) << " ";
...
inline int path_attribute(const char *path)
{
    ...
    { // sometimes files aren't properly determined so this is a fail safe
      // check
...
inline bool delete_path_tree(const char *path)
{
    ...
    if ((strcmp(tempStr.c_str(), "") == 0) || (strcmp(tempStr.c_str(), "NULL") == 0))
        return false;
...
inline void appendFileSuffix(char *oldFileWOExt, char *suffix, string &newFileWOExt)
{
...
inline bool moveFile(char *source,
...
inline bool fileOperation(op command, string &source, string destination = string(), string suf = string())
{
    ...
    cout << "Common: FileIOMacros: inline bool fileOperation(op command: command" << (int)command << '\n';
    exit(0);
}
//=======================================================================================================
...
inline void restoreConsoleOutput()
{
    std::cout.clear();
}
...
class icfilestream : private virtual cfilebuf, public std::istream
{
  public:
    icfilestream(FILE *file)
        : cfilebuf(file), std::ios(static_cast<std::streambuf *>(this)), std::istream(
